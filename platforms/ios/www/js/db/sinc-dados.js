...
function nokTransSinc(erro) {
	alert("Erro ao sincronizar os dados com o servidor! Erro: "+erro.code + ":" + teste);
}
...
function okConsUsuarios(tx, results) {
    ...
    for (var i=0; i<len; i++) {
		...
		$.ajax({
		    type     : "post",
		    url      : "http://intonses.com.br/tagarela/scripts/db/ins-usuario.php",
		    data     : dados,
		    dataType : "json",
...
function selectUsuarios(tx) {
	...
	$.ajax({
	    type     : "post",
	    url      : "http://intonses.com.br/tagarela/scripts/db/sel-usuarios.php",
	    data     : dados,
	    dataType : "json",
	    success  : function(ret) {
	    	if (ret.erro) {
...
function okConsInfo(tx, results) {
    ...
    var cont = 0;
    for (var i=0; i<len; i++) {
    	var urlOpe = "http://intonses.com.br/tagarela/scripts/db/ins-info.php";
		if (results.rows.item(i).sinc == 2) {
			urlOpe = "http://intonses.com.br/tagarela/scripts/db/alt-info.php";
		}
		var dados = {
			"id" : results.rows.item(i).id,
			"usuario" : results.rows.item(i).usuario,
...
function selectInfo(tx) {
	...
	$.ajax({
	    type     : "post",
	    url      : "http://intonses.com.br/tagarela/scripts/db/sel-info.php",
	    data     : dados,
	    dataType : "json",
	    success  : function(ret) {
	    	if (ret.erro) {
...
function okConsObservacoes(tx, results) {
    ...
    for (var i=0; i<len; i++) {
		...
		$.ajax({
		    type     : "post",
		    url      : "http://intonses.com.br/tagarela/scripts/db/ins-observacao.php",
		    data     : dados,
		    dataType : "json",
		    success  : function(ret) {
		    	if (ret.erro) {
...
function selectObservacoes(tx) {
	...
	$.ajax({
	    type     : "post",
	    url      : "http://intonses.com.br/tagarela/scripts/db/sel-observacoes.php",
	    data     : dados,
	    dataType : "json",
	    success  : function(ret) {
	    	if (ret.erro) {
...
function okConsBuilders(tx, results) {
    ...
    for (var i=0; i<len; i++) {
		...
		$.ajax({
		    type     : "post",
		    url      : "http://intonses.com.br/tagarela/scripts/db/ins-builder.php",
		    data     : dados,
		    dataType : "json",
		    success  : function(ret) {
		    	if (ret.erro) {
...
function selectBuilders(tx) {
	...
	$.ajax({
	    type     : "post",
	    url      : "http://intonses.com.br/tagarela/scripts/db/sel-builders.php",
	    data     : dados,
	    dataType : "json",
	    success  : function(ret) {
	    	if (ret.erro) {
...
function okConsConvites(tx, results) {
    ...
    for (var i=0; i<len; i++) {
		...
		$.ajax({
		    type     : "post",
		    url      : "http://intonses.com.br/tagarela/scripts/db/ins-convite.php",
		    data     : dados,
		    dataType : "json",
		    success  : function(ret) {
		    	if (ret.erro) {
...
function selectConvites(tx) {
	...
	$.ajax({
	    type     : "post",
	    url      : "http://intonses.com.br/tagarela/scripts/db/sel-convites.php",
	    data     : dados,
	    dataType : "json",
	    success  : function(ret) {
	    	if (ret.erro) {
...
function okConsLogs(tx, results) {
    ...
    for (var i=0; i<len; i++) {
		...
		$.ajax({
		    type     : "post",
		    url      : "http://intonses.com.br/tagarela/scripts/db/ins-log.php",
		    data     : dados,
		    dataType : "json",
		    success  : function(ret) {
		    	if (ret.erro) {
...
function selectLogs(tx) {
	...
	$.ajax({
	    type     : "post",
	    url      : "http://intonses.com.br/tagarela/scripts/db/sel-logs.php",
	    data     : dados,
	    dataType : "json",
	    success  : function(ret) {
	    	if (ret.erro) {
...
function okConsPlanos(tx, results) {
    ...
    for (var i=0; i<len; i++) {
		...
		$.ajax({
		    type     : "post",
		    url      : "http://intonses.com.br/tagarela/scripts/db/ins-plano.php",
		    data     : dados,
		    dataType : "json",
		    success  : function(ret) {
		    	if (ret.erro) {
...
function selectPlanos(tx) {
	...
	$.ajax({
	    type     : "post",
	    url      : "http://intonses.com.br/tagarela/scripts/db/sel-planos.php",
	    data     : dados,
	    dataType : "json",
	    success  : function(ret) {
	    	if (ret.erro) {
...
function okConsPranchas(tx, results) {
    ...
    for (var i=0; i<len; i++) {
		...
		$.ajax({
		    type     : "post",
		    url      : "http://intonses.com.br/tagarela/scripts/db/ins-prancha.php",
		    data     : dados,
		    dataType : "json",
		    success  : function(ret) {
		    	if (ret.erro) {
...
function selectPranchas(tx) {
	...
	$.ajax({
	    type     : "post",
	    url      : "http://intonses.com.br/tagarela/scripts/db/sel-pranchas.php",
	    data     : dados,
	    dataType : "json",
	    success  : function(ret) {
	    	if (ret.erro) {
...
function okConsSimbolos(tx, results) {
	...
	$.ajax({
	    type     : "post",
	    url      : "http://intonses.com.br/tagarela/scripts/db/count-simbolos.php",
	    data     : dados,
	    dataType : "json",
	    success  : function(ret) {
	    	if (ret.erro) {
...
function baixarSimbolos() {
	...
	$.ajax({
	    type     : "post",
	    url      : "http://intonses.com.br/tagarela/scripts/db/baixar-simbolos.php",
	    data     : dados,
	    dataType : "json",
	    success  : function(ret) {
	    	if (ret.erro) {
...
function localSimbolos(tx) {
	...
	for	(var i = (simbolosNome.length-1); i >= 0; i--) {
        var urlImg = "http://intonses.com.br/tagarela/img/"+simbolosImg[i];
		var localImg = "file:///storage/sdcard0/tagarela/img/"+simbolosImg[i];
		var urlAud = "http://intonses.com.br/tagarela/audio/"+simbolosAudio[i];
...
function okWebSimbolos(tx, results) {
	...
	for (var i=0; i<len; i++) {
		...
		$.ajax({
		    type     : "post",
		    url      : "http://intonses.com.br/tagarela/scripts/db/ins-simbolo.php",
		    data     : dados,
		    dataType : "json",
		    success  : function(ret) {
		    	if (ret.erro) {
...
function uploadSimbolos(tx) {
	...
	for	(var i = 0; i < simbsNomeLocal.length; i++) {
        ...
        opImg.params = params;
    	ftImg.upload(imgLocal, encodeURI("http://intonses.com.br/tagarela/scripts/upload.php"), successUpload, failUpload, opImg);
    	...
    	ftAudio.upload(audioLocal, encodeURI("http://intonses.com.br/tagarela/scripts/upload.php"), successUpload, failUpload, opAudio);
	}
}
...
function consLetrasNumeros(tx) {
    teste = 38;
	tx.executeSql("SELECT id,nome,img,audio,sinc FROM letrasNumeros WHERE sinc <> 0", [], okConsLetrasNumeros, nokTransSinc);
}
function okConsLetrasNumeros(tx, results) {
   	var len = results.rows.length;
    var cont = 0;
    for (var i=0; i<len; i++) {
    	var dados = {
...
function localLetras(tx) {
    ...
    tx.executeSql('CREATE TABLE IF NOT EXISTS letrasNumeros (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, nome TEXT, img TEXT, audio TEXT, sinc INTEGER)');
	for	(var i = 0; i < letrasNome.length; i++) {
        var localImg = "img/"+letrasImg[i];
        var localAud = "audio/"+letrasAudio[i];
		tx.executeSql("INSERT INTO letrasNumeros (nome,img,audio,sinc) VALUES (?,?,?,0)", [letrasNome[i],localImg,localAud]);
	}
}
