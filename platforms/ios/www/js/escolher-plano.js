$(document).ready(function() {
	...
	$.ajax({
	    type     : "post",
	    url      : "http://tagarela.intonses.com.br/scripts/verifica-conexao.php",
	    data     : dados,
	    dataType : "json",
	...
	function okQuerySimbPlanoEsc(tx, results) {
        ...
        for (var i=0; i<len; i++) {
            ...
            if(auxili == 1){
                $(".planos").append("<div class = 'row'>");
            }
	    	if (localStorage.dinamico == "iOS") {
                $(".planos").append("<div class = 'col col-50'>"
                                    + "<a href='#'>"
                                    + "<img src='img/"+results.rows.item(i).img+"' title='"+results.rows.item(i).audio+"'"
                                    + " alt='"+results.rows.item(i).plano+"' class='img-plano' style='margin:25px; border:10px solid "+corBorda+";"
                                    + "height='150' width='150''/></a></div>");
	    	}
	    	else {
	    		$(".planos").append("<div class = 'col col-50' simbs>"
                                    + "<a href='#'>"
